...
namespace MeetAndTalk.Nodes
{
    public class TimerChoiceNode : BaseNode
    {
        ...
        public Port AddChoicePort(BaseNode basenote, DialogueNodePort inDialogueNodePort = null)
        {
            ...
            int outputPortCount = basenote.outputContainer.Query("connector").ToList().Count();
            ...
            {
                dialogueNodePort.HintField = null;
                var newField = new TextField()
                {
                    visible = false
                };

                newField.SetValueWithoutNotify("");
                newField.SetEnabled(false);

                dialogueNodePort.TextField = newField;

                port.contentContainer.Add(newField);
