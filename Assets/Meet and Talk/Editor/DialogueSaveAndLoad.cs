...
namespace MeetAndTalk.Editor
{
    ...
    public class DialogueSaveAndLoad
    {
        ...
        public List<BaseNode> Nodes => graphView.nodes.ToList().Where(node => node is BaseNode).Cast<BaseNode>().ToList();
        ...
        {
            return Nodes.Where(x=> x.GetType() == typeof(T)).Cast<T>().ToList();
        }

        public bool IsEventScriptableObjectPresent(Event.DialogueEventSO eventData)
        {
            var count = 0;
            foreach(EventNode item in GetNodes<EventNode>())
            {
                count += item.EventScriptableObjectDatas.Count(x=>x.DialogueEventSO == eventData);
            }
        ...
        private DialogueChoiceNodeData SaveNodeData(DialogueChoiceNode _node)
        {
            DialogueChoiceNodeData dialogueNodeData = new DialogueChoiceNodeData
            {
                ...
                RequireCharacterInput = _node.RequireInput
            };
        ...
        private DialogueNodeData SaveNodeData(DialogueNode _node)
        {
            DialogueNodeData dialogueNodeData = new DialogueNodeData
            {
                ...
                BackgroundCapable = _node.BackgroundCapable,
                Post = _node.Post,
        ...
        private void GenerateNodes(DialogueContainerSO _dialogueContainer)
        {
            ...
            foreach (DialogueChoiceNodeData node in _dialogueContainer.DialogueChoiceNodeDatas)
            {
                ...
                tempNode.RequireInput = node.RequireCharacterInput;
            ...
            foreach (DialogueNodeData node in _dialogueContainer.DialogueNodeDatas)
            {
                ...
                tempNode.BackgroundCapable = node.BackgroundCapable;
