...
namespace MeetAndTalk.Editor
{
    ...
    public class DialogueGraphView : GraphView
    {
        ...
        /// <summary>
        /// Reloads all the event nodes on the graph and updates their content to current versions
        /// </summary>
        public void ReloadEventNodes()
        {
            List<EventNode> eventNodes = nodes.ToList().Where(node => node is EventNode).Cast<EventNode>().ToList();
            foreach(var item in eventNodes)
            {
                item.RegenerateEventNode();
            }
        }
