...
namespace MeetAndTalk.GlobalValue
{
    public class GlobalValueManager : ScriptableObject
    {
        ...
        public void LoadSaveFile(List<SaveFileData.GlobalSaveVariable> savedVariables)
        {
        ...
        public List<SaveFileData.GlobalSaveVariable> ConvertSaveFile()
        {
            var list = new List<SaveFileData.GlobalSaveVariable>();
            for (int i = 0; i < IntValues.Count; i++)
            {
                list.Add(new SaveFileData.GlobalSaveVariable()
                {
    ...
    static class GlobalValueManagerIMGUIRegister
    {
        ...
        public static SettingsProvider GlobalValueManagerProvider()
        {
            ...
            {
                label = "Global Value Manager",
                guiHandler = (searchContext) =>
                {
                    EditorGUILayout.BeginVertical("helpbox", GUILayout.Height(48));
                    Rect titleRect = EditorGUILayout.GetControlRect();
                    EditorGUI.DrawPreviewTexture(new Rect(titleRect.x - 3, titleRect.y - 3, 48, 48), Resources.Load($"Icon/MT_GlobalValue_Gizmo") as Texture);
                    EditorGUI.LabelField(new Rect(titleRect.x + 50, titleRect.y + 6, titleRect.width - 56, 16), $"Global Value System", EditorStyles.boldLabel);
                    EditorGUI.LabelField(new Rect(titleRect.x + 50, titleRect.y + 22, titleRect.width - 56, 12), $"allows you to add values saved between scenes, you can use them for example: to save level, relationship, character names", EditorStyles.miniLabel);
                    EditorGUILayout.EndVertical();
                    //EditorGUILayout.HelpBox("Global Value System, allows you to add values saved between scenes, you can use them for example: to save level, relationship, character names", MessageType.Info, true);
                    var settings = GlobalValueManager.GetSerializedSettings();

                    ShowArray(settings, "IntValues", "Int", "MT_Int");
                    ShowArray(settings, "FloatValues", "Float", "MT_Float");
                    ShowArray(settings, "BoolValues", "Bool", "MT_Bool");
                    ShowArray(settings, "StringValues", "String", "MT_String");

                    if (GUI.changed)
                    {
                        // Je�li tak, zastosuj zmiany tylko do IntValues
        ...
        public static bool ShowArray(SerializedObject serializedObject, string PropertyName, string objectName, string IconName)
        {
            // Get Property
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                //EditorGUILayout.LabelField(new GUIContent(Resources.Load($"Icon/{IconName}") as Texture));
            ...
            // Button
            if (IconButton("Toolbar Plus", $"Add New {objectName}", GUILayout.Height(EditorGUIUtility.singleLineHeight * 1.5f)))
            {
                property.arraySize++;
                serializedObject.ApplyModifiedProperties();
            }
    ...
    [CustomPropertyDrawer(typeof(GlobalValueSave))]
    public class GlobalValueSaveClassDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            EditorGUI.BeginProperty(position, label, property);
            ...
            index = EditorGUI.Popup(valueNameRect, property.name, index, tmp.ToArray());
            property.FindPropertyRelative("ValueName").stringValue = tmp[index];
    ...
    public class GlobalValueClassDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            for (int i = 0; i < manager.StringValues.Count; i++)
            {
                tmp.Add(manager.StringValues[i].ValueName);
            }
            ...
            {
                if (tmp[i] == property.FindPropertyRelative("ValueName").stringValue)
                {
                    index = i;
                }
            }
            ...
            if (manager.StringValues.Count > 0)
            {
                property.FindPropertyRelative("ValueName").stringValue = tmp[index];
            }
